...
async function getFilenameForLink(link) {
  ...
  const cmd = `yt-dlp -f bestvideo+bestaudio --js-runtimes node --remote-components ejs:github --merge-output-format mp4 --extractor-args "youtube:player_client=default" --get-filename -o "${template}" "${safeLink}"`;
  ...
  if (req.method === 'POST' && req.url === '/download-stream') {
    ...
    req.on('end', async () => {
      try {
        ...
        const args = ['-f', 'bestvideo+bestaudio', '--js-runtimes', 'node', '--remote-components', 'ejs:github', '--extractor-args', '\"youtube:player_client=default\"', '--merge-output-format', 'mp4', '--newline', '-o', absTemplate, safeLink];
  ...
  if (req.method === 'POST' && req.url === '/download') {
    ...
    req.on('end', async () => {
      try {
        ...
        const downloadCmd = `yt-dlp -f bestvideo+bestaudio --js-runtimes node --remote-components ejs:github --merge-output-format mp4 -o "${absTemplate}" "${String(link).replace(/"/g,'\\"')}"`;
