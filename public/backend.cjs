...
async function getFilenameForLink(link) {
  ...
  const cmd = `yt-dlp -f bestvideo+bestaudio --cookies-from-browser firefox --js-runtimes node --remote-components ejs:github --merge-output-format mp4 --extractor-args "youtube:player_client=default" --cookies cookies.txt --get-filename -o "${template}" "${safeLink}"`;
