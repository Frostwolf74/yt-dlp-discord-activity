...
async function getFilenameForLink(link) {
  ...
  const cmd = `yt-dlp -f bestvideo+bestaudio --merge-output-format mp4 --js-runtimes node --remote-components ejs:github --get-filename -o "${template}" "${safeLink}"`;
  ...
  if (req.method === 'POST' && req.url === '/download-stream') {
    ...
    req.on('end', async () => {
      try {
        ...
        const args = ['-f', 'bestvideo+bestaudio', '--js-runtimes', 'node', '--remote-components', 'ejs:github', '--merge-output-format', 'mp4', '--newline', '-o', absTemplate, safeLink];
